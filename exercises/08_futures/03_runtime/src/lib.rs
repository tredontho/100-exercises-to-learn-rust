    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let handle1 = tokio::spawn(reply_work(first, reply.clone()));
    let handle2 = tokio::spawn(reply_work(second, reply));
    tokio::join!(handle1, handle2);
}

async fn reply_work<T>(listener: TcpListener, reply: Arc<T>)
where
    T: Display + Send + Sync + 'static,
{
    loop {
